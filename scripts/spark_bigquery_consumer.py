...
CHECKPOINT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "spark_checkpoints")
...
def main():
    ...
    spark = SparkSession.builder \
        .appName("EcommerceBigQueryStreaming") \
        .config("spark.driver.bindAddress", "127.0.0.1") \
        .config("spark.driver.host", "127.0.0.1") \
        .config("spark.jars.packages", "org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.1,com.google.cloud.spark:spark-3.5-bigquery:0.44.2") \
        .getOrCreate()
    ...
    for topic_name, cfg in topics_config.items():
        ...
        parsed_df = records_str.filter(col("topic") == topic_name) \
            .select(from_json(col("json_str"), cfg["schema"]).alias("data")) \
            .select("data.*")
            
        # Start the query and write to BigQuery using foreachBatch
        query = parsed_df.writeStream \
            .foreachBatch(make_writer(dest_table)) \
            .option("checkpointLocation", checkpoint_path) \
            .start()
            
        queries.append(query)
        
    print(f"🎉 Started {len(queries)} streaming queries. Writing directly to BigQuery raw tables...")
