...
class StatefulEcommerceGenerator:
    def __init__(self):
        ...
        self.max_order_item_idx = self._parse_max_id(ORDER_ITEMS_CSV, "order_item_id", "I")
        ...
        self.review_templates = {
            1: ["Terrible product, broke immediately.", "Waste of money.", "Poor quality, would not buy again."],
            2: ["Color was different from images.", "Item arrived damaged.", "Not as expected, quality is poor."],
            3: ["Decent product for the price.", "Okay quality, average delivery speed.", "Product is fine, nothing special."],
            4: ["Highly recommend this brand.", "Value for money.", "Good product, fits description."],
            5: ["Excellent product, will buy again.", "Super fast shipping!", "Perfect, absolutely love it!"]
        }
    ...
    def _load_cities(self):
        ...
        if not cities:
            cities = {"Taipei", "New Taipei", "Taichung", "Tainan", "Kaohsiung", "Hsinchu"}
        return list(cities)
    ...
    def generate_user(self, date_str):
        ...
        gender = random.choices(["Male", "Female", "Other"], weights=[0.45, 0.45, 0.10], k=1)[0]
    ...
    def generate_review(self, purchase, timestamp_str):
        ...
        rating = random.choices([5, 4, 3, 2, 1], weights=[0.50, 0.30, 0.10, 0.05, 0.05], k=1)[0]
        ...
        try:
            with open(ORDERS_CSV, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if date_str:
                        try:
                            d_part = date_str.split('T')[0] if 'T' in date_str else date_str.split(' ')[0]
...
def reset_data():
    ...
    if not os.path.exists(backup_dir):
        print("❌ Error: Backup directory data/backup/ does not exist. Please run scripts/restore_and_backup.py first.")
        sys.exit(1)
        
    print("🔄 Resetting data files in data/ to original backup...")
    for filename in ["users.csv", "orders.csv", "order_items.csv", "events.csv", "reviews.csv", "products.csv"]:
...
def fill_gap(generator):
    ...
    if latest_date >= yesterday:
        print(f"✅ No gap to fill. Data is already up to date (latest date: {latest_date}).")
        return
    ...
    while current_date <= yesterday:
        ...
        for _ in range(new_users_count):
            ...
            w_users.writerow([user["user_id"], user["name"], user["email"], user["gender"], user["city"], user["signup_date"]])
            
        # 2. Events: 30 to 80 events
        event_count = random.randint(30, 80)
        for _ in range(event_count):
            user_id = random.choice(generator.active_users) if generator.active_users else "U000001"
            ...
            w_events.writerow([event["event_id"], event["user_id"], event["product_id"], event["event_type"], event["event_timestamp"]])
        ...
        for _ in range(order_count):
            ...
            order, items = generator.generate_order_and_items(user_id, random_time.isoformat())
            w_orders.writerow([order["order_id"], order["user_id"], order["order_date"], order["order_status"], order["total_amount"]])
            
            for item in items:
                w_order_items.writerow([item["order_item_id"], item["order_id"], item["product_id"], item["user_id"], item["quantity"], item["item_price"], item["item_total"]])
        ...
        eligible_reviews = [p for p in gap_purchased if p["order_date"] < datetime.combine(current_date, datetime.max.time())]
        if eligible_reviews:
            ...
            for purchase in to_review:
                ...
                review_dt = purchase["order_date"] + timedelta(days=offset, hours=random.randint(0, 5))
                ...
                w_reviews.writerow([
                    review["review_id"], review["order_id"], review["product_id"],
                    review["user_id"], review["rating"], review["review_text"], review["review_date"]
                ])
